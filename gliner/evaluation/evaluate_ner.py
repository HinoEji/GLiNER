...
@torch.no_grad()
def get_for_one_path(path, model, threshold = 0.5, return_preds=False, batch_size = 16):
    ...
    if return_preds:
        results, f1, preds = model.evaluate(
            test_dataset, flat_ner=flat_ner, threshold=threshold, batch_size=batch_size, entity_types=entity_types, return_preds=True
        )
        return data_name, results, f1, preds
    else:
        results, f1 = model.evaluate(
            test_dataset, flat_ner=flat_ner, threshold=threshold, batch_size=batch_size, entity_types=entity_types
        )
